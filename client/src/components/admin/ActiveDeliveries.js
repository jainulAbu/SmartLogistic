...
const ActiveDeliveries = () => {
  ...
  useEffect(() => {
    const deliveriesQuery = query(collection(db, 'deliveries'), where('status', '==', 'active'));
  ...
  let filteredDeliveries = deliveries.filter(delivery =>
    ...
    (!dateRange.from || dayjs(delivery.startDate?.toDate ? delivery.startDate.toDate() : delivery.startDate).isAfter(dayjs(dateRange.from).subtract(1, 'day')))
    && (!dateRange.to || dayjs(delivery.startDate?.toDate ? delivery.startDate.toDate() : delivery.startDate).isBefore(dayjs(dateRange.to).add(1, 'day')))
  );
  ...
  const handleExportCSV = () => {
    ...
    saveAs(blob, 'active_deliveries.csv');
  };
      ...
      <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
        ...
        <Box sx={glassCardStyle}>
          <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, justifyContent: 'space-between', alignItems: { xs: 'stretch', md: 'center' }, mb: 2, gap: 2 }}>
            <Typography variant="h4" sx={{ fontWeight: 700, color: '#fff' }}>Active Deliveries</Typography>
          ...
          <Box mb={3} display="flex" gap={2} flexWrap="wrap">
            ...
            <Chip label={`Active: ${active}`} color="success" />
            <Chip label={`Completed: ${completed}`} color="info" />
              ...
              <TableBody>
                {paginatedDeliveries.map((delivery, idx) => (
                  <TableRow key={delivery.id} selected={isSelected(delivery.id)}>
                    ...
                    <TableCell sx={tableCellStyle}>{delivery.startDate ? dayjs(delivery.startDate?.toDate ? delivery.startDate.toDate() : delivery.startDate).format('YYYY-MM-DD HH:mm') : 'N/A'}</TableCell>
                    <TableCell sx={tableCellStyle}>
                      {statusEditId === delivery.id ? (
                        ...
                        >
                          <MenuItem onClick={() => handleStatusUpdate(delivery.id, 'active')}>Active</MenuItem>
                        ...
                        <Chip
                          icon={delivery.status === 'active' ? <CheckCircleIcon fontSize="small" /> : <CancelIcon fontSize="small" />}
                          label={delivery.status}
                          color={delivery.status === 'active' ? 'success' : 'default'}
                          size="small"
          ...
          <DialogContent>
            {selectedDelivery && (
              <Box>
                ...
                <Typography><b>Start Date:</b> {selectedDelivery.startDate ? dayjs(selectedDelivery.startDate?.toDate ? selectedDelivery.startDate.toDate() : selectedDelivery.startDate).format('YYYY-MM-DD HH:mm') : 'N/A'}</Typography>
        ...
        <Dialog open={activityOpen} onClose={() => setActivityOpen(false)}>
          ...
          <DialogContent>
            ...
            <ul>
              ...
              <li>Driver updated status - 1 hour ago</li>
              <li>Vendor checked progress - 30 minutes ago</li>
            </ul>
          </DialogContent>
          <DialogActions>
